...
func GetRecentActivityHandler(c *gin.Context) {
	...
	apiLogs, err := repository.GetApiLogsByUserID(ctx, user.ID, 1, 10)
	if err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}

	jobLogs, err := repository.GetJobsByUserID(ctx, user.ID, 1, 10)
	if err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}

	response.Success(c, http.StatusOK, "recent activity fetched successfully", gin.H{"apiLogs": apiLogs, "jobLogs": jobLogs})
}
